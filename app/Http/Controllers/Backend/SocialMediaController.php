...
use App\Models\{SocialMedia,TpStatus};
...
class SocialMediaController extends Controller
{
    ...
    public function getSocialMediaPageLoad() {
		...
		$datalist = SocialMedia::join('tp_status', 'social_medias.is_publish', '=', 'tp_status.id')
        ...
        return view($this->theme.'.social-media', compact('statuslist', 'datalist'));
    }
	...
	public function getSocialMediaTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view($this->theme.'.partials.social_media_table', compact('datalist'))->render();
		}
	}
    ...
    public function saveSocialMediaData(Request $request){
		...
		if($id ==''){
			$response = SocialMedia::create($data);
			if($response){
				$res['msgType'] = 'success';
			...
			$response = SocialMedia::where('id', $id)->update($data);
			if($response){
				$res['msgType'] = 'success';
	...
	public function deleteSocialMedia(Request $request){
		...
		if($id != ''){
			$response = SocialMedia::where('id', $id)->delete();
			if($response){
				$res['msgType'] = 'success';
	...
	public function bulkActionSocialMedia(Request $request){
		...
		if($BulkAction == 'publish'){
			$response = SocialMedia::whereIn('id', $idsArray)->update(['is_publish' => 1]);
			if($response){
				$res['msgType'] = 'success';
				$res['msg'] = __('Data Updated Successfully');
			}else{
			...
			$response = SocialMedia::whereIn('id', $idsArray)->update(['is_publish' => 2]);
			if($response){
				$res['msgType'] = 'success';
				$res['msg'] = __('Data Updated Successfully');
			}else{
			...
			$response = SocialMedia::whereIn('id', $idsArray)->delete();
			if($response){
				$res['msgType'] = 'success';
				$res['msg'] = __('Data Removed Successfully');
			}else{
