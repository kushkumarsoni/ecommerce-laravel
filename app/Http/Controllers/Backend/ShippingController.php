...
use App\Models\{Shipping,TpStatus};
...
class ShippingController extends Controller
{
    ...
    public function getShippingPageLoad() {
		...
		$datalist   = Shipping::join('tp_status', 'shippings.is_publish', '=', 'tp_status.id')
                    ->select('shippings.id', 'shippings.title', 'shippings.shipping_fee', 'shippings.desc', 'shippings.is_publish', 'tp_status.status')
                    ->orderBy('shippings.id','desc')
                    ->paginate(10);

        return view($this->theme.'.shipping', compact('statuslist', 'datalist'));
    }
	...
	public function getShippingTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view($this->theme.'.partials.shipping_table', compact('datalist'))->render();
		}
	}
