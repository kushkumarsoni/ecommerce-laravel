...
use App\Models\{Slider,MediaOption};
...
class HomeSliderController extends Controller
{
   ...
   //Slider page load
    public function getSliderPageLoad() {
		...
		$datalist = Slider::join('tp_status', 'sliders.is_publish', '=', 'tp_status.id')
                            ->select('sliders.*', 'tp_status.status')
                            ->orderBy('sliders.id','desc')
                            ->paginate(10);

        return view($this->theme.'.slider', compact('media_datalist', 'statuslist', 'datalist'));
    }
	...
	public function getSliderTableData(Request $request){
		...
		$slider_type = 'home_1'; //$request->slider_type;
		...
		if($request->ajax()){
			...
			return view($this->theme.'.partials.slider_table', compact('datalist'))->render();
		}
	}
