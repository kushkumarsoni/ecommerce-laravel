...
class MenuController extends Controller
{
    ...
    public function getMenuPageLoad(){
		...
		$datalist 		= Menu::join('tp_status', 'menus.status_id', '=', 'tp_status.id')
						->select('menus.id', 'menus.menu_name', 'menus.menu_position', 'menus.lan', 'menus.status_id', 'tp_status.status')
						->orderBy('menus.id','desc')
						->paginate(10);

        return view($this->theme.'.menu', compact('languagelist', 'statuslist', 'datalist'));
    }
	...
	public function getMenuTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view($this->theme.'.partials.menu_table', compact('datalist'))->render();
		}
	}
