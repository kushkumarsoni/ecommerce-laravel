...
$(function () {
	...
	$(document).on('click', '.ProductSubCategories nav ul.pagination a', function(event){
    ...
    $("#product_category_id").trigger("chooser:updated");
...
function onPaginationDataLoad(page) {
	$.ajax({
		url:base_url + "/admin/getProductSubCategoriesTableData?page="+page
		+"&search="+$("#search").val()
...
function onRefreshData() {
	$.ajax({
		url:base_url + "/admin/getProductSubCategoriesTableData?search="+$("#search").val()
		+"&language_code="+$('#language_code').val(),
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
...
function onConfirmWhenAddEdit() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/saveProductSubCategoriesData',
...
function onLoadEditData() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/getProductSubCategoriesById',
		data: 'id='+RecordId,
		success: function (response) {
			var data = response;
			...
			$("#name").val(data.name);
			$("#slug").val(data.slug);
			$("#product_category_id").val(data.product_category_id).trigger("chosen:updated");	
...
function onConfirmDelete() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/deleteProductSubCategories',
		data: 'id='+RecordId,
		success: function (response) {
			var msgType = response.msgType;
			var msg = response.msg;
...
function onConfirmBulkAction() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/bulkActionProductSubCategories',
...
function onCategorySlug() {
	...
	if(strLength>0){
		$.ajax({
			type : 'POST',
			url: base_url + '/admin/hasProductSubCategorySlug',
			data: 'slug='+StrName,
			success: function (response) {
